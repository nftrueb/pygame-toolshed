...
from .font import FontSpriteWriter
...
class UI: 
    ...
    def draw(self, surf: pg.Surface): 
        ...
        for node in self.root.children: 
            # render the node as an image if it has that attribute
            if node.img: 
                img = node.pressed_img if node.filled else node.img
                surf.blit(img, node.bounds) 


            # otherwise render as an outline
            else: 
                width = 1 
